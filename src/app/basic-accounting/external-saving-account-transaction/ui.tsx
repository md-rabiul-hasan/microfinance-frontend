...
const ExternalSavingAccountTransactionPageUi = ({ accounts }: any) => {
    ...
    startTransition(async () => {
      try {
        ...
        modals.openConfirmModal({
          ...
          onConfirm: async () => {
            try {
              const res = await addExternalSavingAccountTransaction(submissionData)
              if (res.success) {
  ...
  const deleteHandler = async (insert_key: string) => {
    modals.openConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const res = await deleteExternalSavingAccountTransaction(insert_key)
          if (res.success) {
  ...
  return (
    <Container fluid>
      <Group justify="space-between" mb="xs">
        <TitleBar title="Basic Accounting || External Savings Account Transaction " url="/" />
      </Group>
        ...
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Paper shadow="xs" p="xs">
            <form onSubmit={form.onSubmit(submit)}>
              <Title order={6} mb="md">
                Make direct transactions into savings account with external organization or Individual (except bank)
              </Title>
...
export default ExternalSavingAccountTransactionPageUi