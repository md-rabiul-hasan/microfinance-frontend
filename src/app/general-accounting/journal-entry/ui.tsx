...
import { getSubAccountHead } from '@actions/general-accounting/journal-entry-config'
...
const JournalEntryPageUi = ({ accounts }: any) => {
  ...
  const [transactionGrid, setTransactionGrid] = useState<TransactionItem[]>([])
        ...
        try {
          ...
          if (res.success) {
            ...
            if (res.data.has_sub_accounts) {
              ...
              setSubAccountList([])
              form.setFieldValue('account_type', '0')
            }
          } else {
  ...
  const getSubAccountName = (accountType: string) => {
  ...
  const addToTransactionGrid = (values: any) => {
    ...
    if (hasSubAccountHead && !values.account_type) {
    ...
    const newTransaction: TransactionItem = {
      ...
      sub_account_name: getSubAccountName(values.account_type),
  ...
  const removeFromTransactionGrid = (id: string) => {
    setTransactionGrid(transactionGrid.filter((item) => item.id !== id))
  }

  const submitAllTransactions = () => {
    startTransition(async () => {
      try {
        // Handle final submission of all transactions
        console.log('All transactions to submit:', {
          transaction_date: form.values.transaction_date,
          remarks: form.values.remarks,
          transactions: transactionGrid
        })

        // Here you would call your API to submit all transactions
        showNotification({
          title: 'Success',
          message: `${transactionGrid.length} transactions submitted successfully`,
          color: 'green'
        })

        // Clear the grid after submission
        setTransactionGrid([])
      } catch (error) {
