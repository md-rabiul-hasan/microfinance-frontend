...
const ExternalSavingAccountListUi = ({ data: { data, pagination } }: any) => {
  ...
  const deleteHandler = (id: number) => {
    modals.openConfirmModal({
      ...
      children: <Text size="sm">Are you sure you want to delete this external saving account?</Text>,
      labels: { confirm: 'Confirm', cancel: 'Cancel' },
      onConfirm: () => {
        startTransition(async () => {
          const res = await deleteExternalSavingAccount(id)
          if (res.success) {
  ...
  return (
    <Container fluid>
      ...
      <Group justify="space-between" mb="xs">
        <TitleBar title="External Savings Account List" url="/" />
        <Group gap="xs">
          <TextInput
            placeholder="Search External Savings Account"
            miw={400}
            value={interSearch}
            ...
            {data?.length > 0 ? (
              data.map((account: any, index: number) => (
                <Table.Tr key={index}>
                  ...
                  <Table.Td>{account.org_name}</Table.Td>
              ...
              <Table.Tr>
                <Table.Td colSpan={5} align="center">
                  External saving account not found
                </Table.Td>
              </Table.Tr>
            )}
          </Table.Tbody>
...
export default ExternalSavingAccountListUi
